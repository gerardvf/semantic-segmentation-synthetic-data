...
def compute_val_iou_syn(val_batch_size):  # mean IoU on validation set (val_batch_size should be a divisor of the number of val images)
    ...
    num_val_batches_per_epoch = len(synscapes.val_paths) // val_batch_size
    for batch_id in range(num_val_batches_per_epoch):
        x, y_true = synscapes.get_validation_batch(batch_id, val_batch_size)  # (bs, 512, 1024, 3), (bs, 512, 1024)
...
weights_path = './raid_storage/weightssyn'
...
iou_mean_syn = compute_val_iou_syn(val_batch_size=4)
iou_mean_city = compute_val_iou_city(val_batch_size=4)
print(f'[Epoch 0/{num_epochs}]. Validation mIoU: {iou_mean_syn:.4f} {iou_mean_city:.4f}')
for epoch_id in range(num_epochs):
    synscapes.shuffle_training_paths()
    for batch_id in range(num_batches_per_epoch):
        x, y_true = synscapes.get_training_batch(batch_id, batch_size)
    ...
    iou_mean_syn = compute_val_iou_syn(val_batch_size=4)
    iou_mean_city = compute_val_iou_city(val_batch_size=4)
    print(f'[Epoch {epoch_id+1}/{num_epochs}]. Validation mIoU: {iou_mean_syn:.4f} {iou_mean_city:.4f}')
