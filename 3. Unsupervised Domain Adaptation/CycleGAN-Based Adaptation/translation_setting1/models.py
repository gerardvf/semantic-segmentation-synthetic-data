...
class Discriminator(tf.keras.Model):
    ...
    def call(self, input):
        x = tf.nn.leaky_relu(self.conv1(input))  # default alpha = 0.2

        x = tf.nn.leaky_relu(self.in2(self.conv2(x)))
        
        x = tf.nn.leaky_relu(self.in3(self.conv3(x)))

        x = tf.pad(x, [[0, 0], [1, 1], [1, 1], [0, 0]], mode='CONSTANT')  # this is wrong in the keras implementation
    ...
    def plot(self):
        input = tf.keras.Input(shape=[452, 452, 3])
        model = tf.keras.Model(inputs=input, outputs=self.call(input))
