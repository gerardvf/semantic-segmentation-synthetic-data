...
class Dataset:
    ...
    def get_training_batch(self, batch_id, batch_size):
        ...
        for i in range(batch_id*batch_size, (batch_id+1)*batch_size):
            ...
            if i >= batch_id*batch_size + batch_size//2:  # augment with pbn only half of the batch
                random_class_colors = tf.random.uniform(shape=[20, 3], minval=-1., maxval=1.)  # because image is already in [-1, 1]
                colored_map = tf.gather(random_class_colors, map)
                alpha = tf.random.uniform([], minval=0.5, maxval=0.99)
                image = (1 - alpha) * image + alpha * colored_map
