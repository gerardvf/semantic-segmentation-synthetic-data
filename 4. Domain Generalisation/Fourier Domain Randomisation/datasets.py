...
class Dataset:
    ...
    def get_training_batch(self, batch_id, batch_size, fourier_mask, style_paths):
        images, maps = [], []
        ...
        for i in range(batch_id*batch_size, (batch_id+1)*batch_size):
            ...
            random_style_image = read_image(random.choice(style_paths))
            image = fourier_style_transfer(image, random_style_image, fourier_mask)
            # tf.io.write_file(image_path.replace('synscapes', 'textured'), tf.image.encode_jpeg(image, quality=100, chroma_downsampling=False))
